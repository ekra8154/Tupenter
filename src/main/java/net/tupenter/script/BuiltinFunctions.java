    ...
    private static List<Doc> buildAll() {
        return List.of(
                ...
                doc("block", Group.WORLD, "block(x, y, z) · block(\"x y z\")",
                        ...
                        "§7Decimals floor§r (block coordinates always do), and an unloaded chunk errors LOUDLY — guard it with loaded(...) if it might be out of view.",
                ...
                doc("loaded", Group.WORLD, "loaded(x, y, z) · loaded(\"x y z\")",
                        "is that position in a chunk your client has? true / false — never errors",
                        "#if (loaded(client.blockpos)) (/echo standing in a loaded chunk)",
                        "#if (loaded(grave)) (/echo $block(grave)$) #else (/echo grave chunk is out of view)",
                        "§7The guard for block(...):§r block errors on an unloaded chunk; loaded(...) is the test that doesn't, so #if (loaded(p)) (block(p) ...) is safe.",
                        "§7Render, not simulation:§r loaded(...) is what your client has DRAWN. For whether the server is actually TICKING there, use simulated(...)."),
                doc("simulated", Group.WORLD, "simulated(x, y, z) · simulated(\"x y z\")",
                        ...
                        "§7What despawn/spawning obey:§r a chunk ticks entities (items despawn, mobs spawn, crops grow) only within the server's SIMULATION distance of you — often smaller than render, so loaded(p) can be true while simulated(p) is false.",
