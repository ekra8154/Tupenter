    ...
    private static List<Doc> buildAll() {
        return List.of(
                ...
                doc("int", Group.MATH, "int(x)",
                        ...
                        "§7Takes text too:§r like float, it's an explicit conversion — int(\"42\") is 42, so a substr or an NBT string tag can become a whole number."),
                doc("float", Group.MATH, "float(x)",
                        "x as a number — turns numeric text into math material",
                        "/calc float(\"3.5\") * 2",
                        "/calc float(substr(\"level_42\", 6)) + 1",
                        "§7Text to number:§r substr results and NBT STRING tags are text — float(\"3.5\") is the NUMBER 3.5, ready for * and <. (Numeric NBT tags already come back as numbers; this is for the ones that don't.)",
                        "§7The only coercion in the language:§r int and float are the explicit conversions, so everywhere ELSE text stays text — which is why \"3\" + 1 concatenates instead of quietly adding.",
                ...
                doc("list", Group.LISTS, "list(a, b, ...)",
                        ...
                        "§7Works anywhere:§r the (a | b | c) form only parses in a #foreach header, so list(...) is how a list goes into a #local or #set.",
                ...
                doc("blockpos", Group.VECTORS, "blockpos(x, y, z) · blockpos(v)",
                        ...
                        "§7Whole numbers:§r blockpos(10.7, 64.2, -3.4) is 10 64 -4. /setblock and /fill reject decimals, so this is what makes a computed position usable.",
                        "§7Floors, like client.blockpos:§r the block a position is INSIDE. round(...) picks the NEAREST block instead — half a block away, and the right answer when you're plotting geometry.",
                        "§7One vec3 in:§r blockpos(raycast(50)) says what three round(component(v, \"x\")) calls used to. Already whole? It changes nothing."),
                ...
                doc("vadd", Group.VECTORS, "vadd(a, b)",
                        ...
                        "§7Point plus offset:§r vadd(position, offset) moves a point — the offset is usually scale(direction, distance).",
                        "§7The inverse is vsub.§r Together with scale they build any vector expression."),
                doc("vsub", Group.VECTORS, "vsub(a, b)",
                        ...
                        "§7Direction from a to b:§r vsub(target, here) points from here to target — normalize it for a heading.",
                        "§7Displacement:§r mag(vsub(a, b)) is another way to spell dist(a, b)."),
                doc("scale", Group.VECTORS, "scale(v, s)",
                        "every component times the number s",
                        "/calc scale(vec(1, 2, 3), 10)",
                        "/tp @s $vadd(client.pos, scale(client.look, 5))$",
                        "§7Heading into a step:§r scale(direction, distance) turns a unit heading into an actual move.",
                        "§7Negative flips it:§r scale(v, -1) is the reverse vector."),
                doc("mag", Group.VECTORS, "mag(v)",
                        "the length of a vector",
                        "/calc mag(vec(3, 4, 0))",
                        "/echo moving $mag(client.motion)$ blocks/tick",
                        "§7Length from the origin:§r mag(v) is dist(v, vec(0, 0, 0)) — the size of a displacement or velocity.",
                        "§7Not for points:§r the gap between two POSITIONS is dist(a, b)."),
                doc("dist", Group.VECTORS, "dist(a, b)",
                        "straight-line distance between two points",
                        "/calc dist(vec(0, 0, 0), vec(3, 4, 0))",
                        "/echo $dist(client.pos, world.spawn)$ blocks from spawn",
                        "§7The everyday one:§r how far apart two vec3s are — no more sqrt((a.x-b.x)^2 + ...) by hand.",
                        "§7Points, not directions:§r for the length of a motion or displacement vector, that's mag(v)."),
                doc("normalize", Group.VECTORS, "normalize(v)",
                        "the same direction, scaled to length 1",
                        "/calc normalize(vec(0, 0, 5))",
                        "/tp @s $vadd(client.pos, scale(normalize(client.look), 3))$",
                        "§7A pure direction:§r strips the length, keeps the heading — the building block for \"N blocks that way\".",
                        "§7Zero stays zero:§r normalize(vec(0, 0, 0)) is \"0 0 0\", not an error, so a still player never faults a tick script."),
                doc("dot", Group.VECTORS, "dot(a, b)",
                        ...
                        "§7How aligned two vectors are:§r positive = same-ish way, 0 = perpendicular, negative = opposite.",
                        "§7The \"am I facing it\" test:§r dot(look, directionToThing) > 0 means it's ahead of you.",
                        "§7Exact:§r keeps full precision (no sqrt involved)."),
                doc("cross", Group.VECTORS, "cross(a, b)",
                        ...
                        "§7A sideways vector:§r perpendicular to both inputs — the way to build a plane around a direction (the rainbow-ring trick).",
                        "§7Order matters:§r cross(a, b) points opposite cross(b, a).",
                        "§7Exact:§r full precision, no rounding."),
                ...
                doc("block", Group.WORLD, "block(x, y, z) · block(\"x y z\")",
                        ...
                        "§7Decimals floor§r (block coordinates always do). A chunk your client hasn't received reads as \"minecraft:void_air\" — test for that to tell \"out of view\" from a real block.",
                ...
                doc("simulated", Group.WORLD, "simulated(x, y, z) · simulated(\"x y z\")",
                        ...
                        "§7What despawn/spawning obey:§r a chunk ticks entities (items despawn, mobs spawn, crops grow) only within the server's SIMULATION distance of you — often smaller than render, so a chunk you can still SEE may be frozen.",
                        "§7Only your seat:§r it uses the server's simulation distance (the F3 value) and YOUR position. On a shared server it can't see a distant teammate who is also keeping the chunk ticking."),
                ...
                doc("keys", Group.ENTITIES, "keys(selector, path)",
                        ...
                        "§7Addresses, not values:§r a compound gives its KEYS (sorted), a list gives its indices. Feed one back through entity(...) to read it: entity(\"self\", \"nbt.Inventory.\" + i + \".id\").",
                        "§7Absent = empty:§r a path that isn't there yields an EMPTY list, not an error — an unenchanted item has no minecraft:enchantments key, and the useful answer is \"none\". So len() is 0 and contains() is false, safe in a tick script.",
                        "§7What it unlocks:§r compounds keyed by ID, where you can't know the members in advance — item components, enchantments, a mob's nbt.Brain.memories. It's also the only way to loop an NBT LIST: #foreach $i$ in keys(\"self\", \"nbt.Inventory\").",
                        "§7Path form§r is entity(...)'s: \"nbt.<path>\", or just \"nbt\" for the whole entity. Browse the tree with /tupenter dump."),
