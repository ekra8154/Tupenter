...
public final class DirectiveDocs {
    ...
    private static List<Doc> buildAll() {
        return List.of(
                // ---- Variables ----
                doc("set", Group.VARIABLES, "#set name[:type] = value",
                        ...
                        "§7Optional type:§r #local c:blockpos = blockpos(-10, 20, 85) — the same keywords a custom command's <name:type> params take. It CHECKS the value (three whole numbers, or the line stops) and it tells the chat bar the SHAPE before the value exists, so /tp $c$ keeps completing the rest of the line. /customcommand help types lists them."),
                doc("local", Group.VARIABLES, "#local name[:type] = value",
                        ...
                        "§7Optional type:§r #local c:blockpos = blockpos(-10, 20, 85) — the same keywords a custom command's <name:type> params take. It CHECKS the value (three whole numbers, or the line stops) and it tells the chat bar the SHAPE before the value exists, so /tp $c$ keeps completing the rest of the line. /customcommand help types lists them."),
                doc("setdefault", Group.VARIABLES, "#setdefault name[:type] = value",
                ...
                doc("foreach", Group.LOOPS, "#foreach $x$ in (a | b | c) (body) · #foreach $x$ in <list> (body)",
                        ...
                        "§7Two sources:§r (a | b | c) options written in place, or any list — range(1, 10), list(...), registry sets, entities(radius), client.effects.",
                        "§7(a | b | c) is not special here§r — it is an ordinary list value and works anywhere an expression does, so #local kinds = (short | tall) then #foreach $k$ in kinds is the same loop. Its items are literal TEXT (bare words need no quotes); list(a, b) is the form whose items are computed.",
